...
class CreateCreditmemoStepItemItemSimpleComponent extends CreateCreditmemoStepItemItemAbstractComponent {
    static className = 'CreateCreditmemoStepItemItemSimpleComponent';
    ...
    setReturnStockMessageElementValue(newCreditmemoItemParam) {
        ...
        if (newCreditmemoItemParam.can_return_stock) {
            if (
                ...
                && ![ProductTypeConstant.VIRTUAL].includes(newCreditmemoItemParam.order_item.product_type)
            ) {
    ...
    getQty() {
        ...
        /*return (this.state.creditmemo_item_param.qty || 0).toString().replace(".", this.state.decimal_symbol);*/
        return this.state.creditmemo_item_param.is_qty_decimal ?
            CurrencyHelper.formatNumberStringToCurrencyString(qty, this.props.order.order_currency_code) :
            qty;
    }
    ...
    canReturnStock() {
        ...
        if (
            creditmemoItemParam.qty <= creditmemoItemParam.non_shipped_qty_to_return
            && ![ProductTypeConstant.VIRTUAL].includes(creditmemoItemParam.order_item.product_type)
        ) {
            return false;
        }
        return true;
...
class CreateCreditmemoStepItemItemSimpleContainer extends CoreContainer {
    static className = 'CreateCreditmemoStepItemItemSimpleContainer';
}

/**
 * @type {CreateCreditmemoStepItemItemSimpleContainer}
 */
export default ContainerFactory.get(CreateCreditmemoStepItemItemSimpleContainer).withRouter(
    ComponentFactory.get(CreateCreditmemoStepItemItemSimpleComponent)
)